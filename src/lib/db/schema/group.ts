import { pgTable, uuid, text, timestamp, decimal, integer, unique, index, check } from 'drizzle-orm/pg-core';
import { groupTypeEnum, groupRoleEnum, memberStatusEnum, messageTypeEnum } from './enums';
...
export const groups = pgTable('group', {
  ...
  subscriptionCheck: check('subscription_check', "(group_type = 'subscription' AND subscription_price IS NOT NULL AND stores_price_id IS NOT NULL) OR (group_type != 'subscription' AND subscription_price IS NULL AND stores_price_id IS NULL)")
}));
