...
export const schedulePolls = pgTable('schedule_poll', {
  id: uuid('id').primaryKey().defaultRandom(),
...
export const scheduleCandidates = pgTable('schedule_candidate', {
  id: uuid('id').primaryKey().defaultRandom(),
  pollId: uuid('poll_id').notNull().references(() => schedulePolls.id, { onDelete: 'cascade' }),
...
// Schedule vote table
export const scheduleVotes = pgTable('schedule_vote', {
  id: uuid('id').primaryKey().defaultRandom(),
  pollId: uuid('poll_id').notNull().references(() => schedulePolls.id, { onDelete: 'cascade' }),
  candidateId: uuid('candidate_id').notNull().references(() => scheduleCandidates.id, { onDelete: 'cascade' }),
  userId: uuid('user_id').notNull().references(() => profiles.id, { onDelete: 'cascade' }),
