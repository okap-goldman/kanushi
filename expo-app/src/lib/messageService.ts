...
import { ApiResponse, Message, Conversation, UserProfile } from './data';

// Define additional types for the messaging feature
...
// Extend the Message and Conversation types from data.ts
...
): Promise<ApiResponse<ExtendedConversation[]>> => {
  try {
      ...
      conversationsData.map(async (conversation) => {
        ...
        return {
          ...
          display_name: otherParticipants.length === 1 
            ...
            : formattedParticipants.map((p: ConversationParticipant) => p.user.name).join(', '),
    ...
    return { data: conversationsWithLastMessage as ExtendedConversation[], error: null };
  } catch (error) {
...
): Promise<ApiResponse<{ conversation: ExtendedConversation; messages: ExtendedMessage[]; has_more: boolean }>> => {
  try {
    ...
    return { 
      data: { 
        conversation: conversation as ExtendedConversation, 
        messages: formattedMessages as ExtendedMessage[],
        has_more: hasMore
      }, 
      error: null 
    };
...
export const sendMessage = async (
  message: Omit<ExtendedMessage, 'id' | 'created_at' | 'sender' | 'receiver' | 'reactions'>
): Promise<ApiResponse<ExtendedMessage>> => {
  try {
...
export const reactToMessage = async (
  ...
  user_id: string,
  reaction: string
): Promise<ApiResponse<MessageReaction | null>> => {
  try {
...
export const getAvailableUsers = async (
  ...
  try {
    let query = supabase
      .from('profiles')
      .select('id, name, image, username, bio, followers_count, following_count, posts_count, created_at');
