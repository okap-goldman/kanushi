...
import { type SendMessageResponse, aiChatService } from '../../src/lib/aiChatService';
...
describe('aiChatService', () => {
  ...
  describe('User Query Submission', () => {
    it('should send user query and receive AI response', async () => {
      const mockResponse = {
        message:
          '私はあなたの精神的な成長をサポートするAIアシスタントです。どのようなご質問でもお気軽にどうぞ。',
  ...
  describe('Timeout Handling', () => {
    it('should handle timeout for long-running operations', async () => {
      ...
      await expect(
        Promise.race([aiChatService.sendMessage('複雑な質問', { timeout: 100 }), timeoutPromise])
  ...
  describe('Inappropriate Content Filtering', () => {
    it('should filter inappropriate content in user messages', async () => {
      ...
      await expect(aiChatService.sendMessage('不適切な内容を含むメッセージ')).rejects.toThrow(
