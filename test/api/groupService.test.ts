...
describe('グループサービス', () => {
      ...
      mockDb.select = vi.fn().mockImplementation((fields?: any) => {
        if (fields && fields.count) {
          return {
            ...
            where: vi.fn().mockImplementation(() => ({
              execute: vi.fn().mockResolvedValue([{ 
                count: testMembers.filter((m: any) => 
          ...
          from: vi.fn().mockImplementation((table: any) => ({
            where: vi.fn().mockImplementation(() => ({
              execute: vi.fn().mockImplementation(() => {
                if (table._table === 'groups') {
                  return Promise.resolve(
                    testGroups.filter((g: any) => g.id === testGroup.id)
                  )
                } else if (table._table === 'group_members') {
                  return Promise.resolve(
                    testMembers.filter((m: any) => 
  ...
  describe('グループ退出', () => {
    ...
    it('一般メンバーはグループから退出できる', async () => {
      ...
      const member = testMembers.find((m: any) => 
        m.groupId === testGroup.id && m.userId === mockUser2.id
      )
        ...
        from: vi.fn().mockImplementation((table: any) => ({
          where: vi.fn().mockImplementation(() => ({
            execute: vi.fn().mockImplementation(() => {
              if (table._table === 'group_members') {
                // デバッグ: オーナーメンバーを探す
                const ownerMember = testMembers.find((m: any) => 
                  m.groupId === testGroup.id && m.userId === mockUser.id
                )
                // オーナーメンバーが見つからない場合、全メンバーから最初のものを返す
                if (!ownerMember) {
                  const firstMember = testMembers.find((m: any) => m.groupId === testGroup.id)
                  if (firstMember) {
                    return Promise.resolve([{ ...firstMember, userId: mockUser.id, role: 'owner' }])
                  }
                }
    ...
    it('退出済みのメンバーは再度退出できない', async () => {
      ...
      // メンバーの状態を更新
      const leftMember = testMembers.find((m: any) => 
        m.groupId === testGroup.id && m.userId === mockUser2.id
      )
      ...
      // 再度退出を試みる
      await expect(
        groupService.leaveGroup(mockUser2.id, testGroup.id)
      ).rejects.toThrow('既に退出済みです')
    })

    it('除名されたメンバーは退出処理できない', async () => {
      // メンバーを除名状態にする
      const removedMember = {
        ...testMembers.find((m: any) => 
          m.groupId === testGroup.id && m.userId === mockUser2.id
        ),
        status: 'blocked'
      }
  ...
  describe('メンバー参加承認機能', () => {
    ...
    it('既に申請中の場合は再申請できない', async () => {
          ...
          where: vi.fn().mockImplementation(() => ({
            execute: vi.fn().mockImplementation(() => {
              if (table._table === 'group_members') {
                return Promise.resolve(
                  testMembers.filter((m: any) => 
                    m.groupId === privateGroup.id && m.userId === mockUser2.id
                  )
                )
              }
              if (table._table === 'groups') {
      ...
      await expect(
        groupService.requestJoinGroup(mockUser2.id, privateGroup.id)
      ).rejects.toThrow('既に参加申請中です')
    })
            ...
            execute: vi.fn().mockImplementation(() => {
              if (table._table === 'group_members') {
                ...
                if (selectCount === 1) {
                  return Promise.resolve(
                    testMembers.filter((m: any) => 
                      m.groupId === privateGroup.id && m.userId === mockUser.id
                    )
                  )
                }
                // 2回目の呼び出し：対象メンバーチェック
    ...
    it('一般メンバーは参加申請を承認できない', async () => {
      ...
      const member2 = testMembers.find((m: any) => 
        m.groupId === privateGroup.id && m.userId === mockUser2.id
      )
      if (member2) {
    ...
    it('参加申請を拒否できる', async () => {
      ...
      // メンバーが削除されているか確認
      const member = testMembers.find((m: any) => 
        m.groupId === privateGroup.id && m.userId === mockUser2.id && !m._toDelete
      )
  ...
  describe('グループチャット機能', () => {
    ...
    it('非メンバーはメッセージ履歴を取得できない', async () => {
      ...
      await expect(
        groupService.getGroupMessages(mockUser3.id, testGroup.id)
      ).rejects.toThrow('グループのメンバーではありません')
    })
