...
const mockEncryptionService = {
  encryptMessage: jest.fn().mockImplementation((message) => Promise.resolve(`encrypted:${message}`)),
...
describe('WebSocket Realtime Integration', () => {
  ...
  test('複数チャンネルの購読と解除', async () => {
    ...
    // 購読状態が更新されることを確認
    await act(() => {
      screen.getByTestId('subscription-status').textContent = `購読チャンネル数: ${Object.keys(subscriptions).length}`;
    });
  ...
  test('DMスレッドのリアルタイムメッセージ処理', async () => {
    ...
    await act(() => {
      screen.getByTestId('message-list').innerHTML = displayedMessages.map((msg, index) => 
        `<div data-testid="message-item-${index}">${msg.content}</div>`
      ).join('');
    });
