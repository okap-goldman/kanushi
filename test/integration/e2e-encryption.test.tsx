...
describe('E2E Encryption Integration', () => {
  ...
  test('エンドツーエンド暗号化のキー生成と管理', async () => {
    ...
    // 生成されたキーが保存されることを確認
    expect(mockEncryptionService.storeKeyPair).toHaveBeenCalledWith('test-user', {
      publicKey: 'mock-public-key',
      privateKey: 'mock-private-key',
    });
  ...
  test('暗号化・復号プロセスの完全性検証', async () => {
    ...
    mockEncryptionService.decryptMessage.mockImplementation((encryptedMessage, privateKey) => {
      ...
      // 対応する秘密鍵で復号可能かチェック
      if (
        (publicKey === 'recipient-public-key' && privateKey === recipientKeyPair.privateKey) ||
    ...
    // 誤った秘密鍵では復号できないことを確認
    mockEncryptionService.decryptMessage.mockImplementationOnce((encryptedMsg, privateKey) => {
    ...
    await expect(
      mockEncryptionService.decryptMessage(encryptedMessage, 'wrong-private-key')
  ...
  test('前方秘匿性を保証する鍵のローテーション', async () => {
    ...
    mockEncryptionService.storeKeyPair.mockImplementation((userId, keyPair) => {
      keyHistory.push({
        timestamp: Date.now(),
        keyPair,
      });
      ...
      return (
        <div data-testid="key-rotation-screen">
          ...
          <div data-testid="key-info">
            現在の鍵:{' '}
            {keyHistory.length > 0 ? keyHistory[keyHistory.length - 1].keyPair.publicKey : 'なし'}
          </div>
        </div>
      );
    }
